...
def regenerate_cars(num_cars):
    ...
    # 更新现有汽车的目标位置
    if len(cars) > 0:
        points = random_points(len(cars), min_distance=50, screen_width=screen_width, screen_height=screen_height)
        for i, car in enumerate(cars):
            if i < len(points):
                car.set_target(points[i][0], points[i][1])

    # 如果现有的汽车数量超过num_cars，则将多余的汽车目标设置到屏幕外
    if len(cars) > num_cars:
        for i in range(num_cars, len(cars)):
            cars[i].set_target(screen_width + 200, screen_height + 200)

    # 如果现有的汽车数量少于num_cars，则生成新的汽车
    elif len(cars) < num_cars:
        points = random_points(num_cars - len(cars), min_distance=50, screen_width=screen_width, screen_height=screen_height)
        for point in points:
            x, y = point
            car = Car(x=x, y=y)
